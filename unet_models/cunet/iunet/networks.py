...
class iUNet(nn.Module):
    ...
    def __init__(
        ...
        dim: int,
        create_module_fn: CreateModuleFnType = create_standard_module,
        ...
        if slice_mode is "double" or slice_mode is "constant":
            ...
            for i in range(len(architecture) - 1):
                ...
                self.channels.append(
                    self.channel_multipliers[i] * self.channels_before_downsampling[i]
                )
        else:
            raise AttributeError(
                "Currently, only slice_mode='double' and 'constant' are " "supported."
            )
        ...
        for i, num_layers in enumerate(architecture):
            ...
            if i < len(architecture) - 1:
                ...
                self.slice_layers.append(
                    InvertibleModuleWrapper(
                        SplitChannels(self.skipped_channels[i]),
                        disable=disable_custom_gradient,
                    )
                )
                self.concat_layers.append(
                    InvertibleModuleWrapper(
                        ConcatenateChannels(self.skipped_channels[i]),
                        disable=disable_custom_gradient,
                    )
                )
                ...
                self.downsampling_layers.append(
                    InvertibleModuleWrapper(downsampling, disable=learnable_resampling)
                )

                self.upsampling_layers.append(
                    InvertibleModuleWrapper(upsampling, disable=learnable_resampling)
                )
            ...
            for j in range(num_layers):
                ...
                self.encoder_modules[i].append(
                    InvertibleModuleWrapper(
                        create_module_fn(
                            self.channels[i], **coordinate_kwargs, **module_kwargs
                        ),
                        disable=disable_custom_gradient,
                    )
                )
                ...
                if (
                    self.channel_mixing_freq > 0
                    and modulo == self.channel_mixing_freq - 1
                ):
                    add_channel_mixing(self, self.encoder_modules[i])
                    add_channel_mixing(self, self.decoder_modules[i])
