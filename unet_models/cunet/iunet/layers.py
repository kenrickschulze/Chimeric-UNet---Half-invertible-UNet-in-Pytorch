...
def __calculate_kernel_matrix_givens__(weight, **kwargs):
    raise NotImplementedError(
        "Parametrization via Givens rotations not " "implemented."
    )


def __calculate_kernel_matrix_bjork__(weight, **kwargs):
    raise NotImplementedError(
        "Parametrization via Bjork peojections " "not implemented."
    )
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.weight = torch.nn.Parameter(
            __initialize_weight__(
                kernel_matrix_shape=self._kernel_matrix_shape,
                stride=self.stride,
                method=self.method,
                init=init,
                **self.init_kwargs
...
class StandardBlock(nn.Module):
    def __init__(
        self, dim, num_in_channels, num_out_channels, block_depth=1, zero_init=True
    ):
        ...
        for i in range(block_depth):
            ...
            self.seq.append(
                conv_op(
                    current_in_channels, current_out_channels, 3, padding=1, bias=False
                )
            )
            torch.nn.init.kaiming_uniform_(
                self.seq[-1].weight, a=0.01, mode="fan_out", nonlinearity="leaky_relu"
            )
        ...
        # becomes an identity transform (up to permutation
        # of channels)
        if zero_init:
...
def create_standard_module(in_channels, **kwargs):
    ...
    return StandardAdditiveCoupling(
        F=StandardBlock(
            dim, num_F_in_channels, num_F_out_channels, block_depth=block_depth
        ),
        channel_split_pos=num_F_out_channels,
    )


def __initialize_weight__(
    kernel_matrix_shape: Tuple[int, ...],
    stride: Tuple[int, ...],
    method: str = "cayley",
    init: str = "haar",
    dtype: str = "float32",
    ...
    **kwargs
    ...
    if method is "householder":
        warn(
            "Householder parametrization not fully implemented yet. "
    ...
    if init is "haar" and set(stride) == {2}:
        if method == "exp":
            ...
            if dim == 2:
                weight = np.array(
                    [[[0, 0, p, p], [0, 0, -p, -p], [0, 0, 0, 0], [0, 0, 0, 0]]],
                    dtype=dtype,
                )
            if dim == 3:
                weight = np.array(
...
class OrthogonalChannelMixing(nn.Module):
    def __init__(
        self, in_channels: int, method: str = "cayley", learnable: bool = True, **kwargs
    ):
        super(OrthogonalChannelMixing, self).__init__()

        self.in_channels = in_channels
        self.weight = nn.Parameter(
            torch.zeros((in_channels, in_channels)), requires_grad=learnable
        )
...
class InvertibleChannelMixing1D(OrthogonalChannelMixing):
    def __init__(
        ...
        super(InvertibleChannelMixing1D, self).__init__(
            in_channels=in_channels, method=method, learnable=learnable, **kwargs
        )
        self.kwargs = kwargs
